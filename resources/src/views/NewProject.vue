  ...
  <div class="max-w-4xl mx-auto">
    <div class="mb-8">
      <nav class="flex mb-4 text-sm text-slate-500 gap-2 items-center">
        ...
        <span class="text-slate-900 dark:text-slate-100">Tạo dự án mới</span>
      </nav>
      <h1 class="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100">Tạo dự án mới</h1>
      ...
      <form class="p-6 md:p-8 space-y-8" @submit.prevent="submit">
        ...
        <section class="grid grid-cols-1 md:grid-cols-1 gap-6">
          <div class="flex flex-col gap-2">
            ...
            <input v-model="form.name" class="rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-primary focus:border-primary px-4 py-2.5" placeholder="e.g. AI-Powered Analytics Dashboard" type="text"/>
          </div>
        </section>
        <!-- Dates and Status -->
        <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="flex flex-col gap-2">
            ...
            <select v-model="form.status" class="rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-primary focus:border-primary px-4 py-2.5">
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="relative">
              <input v-model="form.start_date" class="w-full rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-primary focus:border-primary px-4 py-2.5" type="date"/>
            </div>
          </div>
          <div class="flex flex-col gap-2">
            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Ngày kết thúc</label>
            <div class="relative">
              <input v-model="form.end_date" class="w-full rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-primary focus:border-primary px-4 py-2.5" type="date"/>
            </div>
          </div>
        </section>
        <!-- Description -->
        <section class="flex flex-col gap-2">
          ...
          <div class="border border-slate-300 dark:border-slate-700 rounded-lg overflow-hidden">
            <div class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-300 dark:border-slate-700 px-3 py-2 flex gap-2">
              <button class="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded" type="button"><span class="material-symbols-outlined text-xl">format_bold</span></button>
              <button class="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded" type="button"><span class="material-symbols-outlined text-xl">format_italic</span></button>
              <button class="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded" type="button"><span class="material-symbols-outlined text-xl">format_list_bulleted</span></button>
              <button class="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded" type="button"><span class="material-symbols-outlined text-xl">link</span></button>
            </div>
            <textarea v-model="form.description" class="w-full border-none bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-0 p-4 resize-none" placeholder="Enter project scope, goals, and key deliverables..." rows="6"></textarea>
          </div>
        </section>
        <!-- Team Assignment -->
        ...
        <div class="flex items-center justify-end gap-4 pt-6">
          ...
          <button class="px-8 py-2.5 rounded-lg bg-primary text-white font-bold shadow-lg shadow-primary/30 hover:bg-primary/90 transition-all" type="submit">
            Create Project
          </button>
        </div>
      </form>
    ...
    <div class="mt-8 p-4 bg-primary/5 rounded-lg border border-primary/10 flex items-start gap-4">
      ...
      <div>
        ...
        <p class="text-xs text-slate-500 mt-1">Once created, project participants will receive an automated invitation email with their assigned roles and access to the project board.</p>
      </div>
    </div>
  </div>
</template>
