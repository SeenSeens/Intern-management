 ...
 */
namespace InternManagement\Modules\Auth;
...
final class Auth {
    ...
    private function define_constants(): void{
        if(!defined('INTERN_MANAGEMENT_AUTH_FILE')) {
            define('INTERN_MANAGEMENT_AUTH_FILE', __FILE__);
        }
        if(!defined('INTERN_MANAGEMENT_AUTH_URL')) {
            define('INTERN_MANAGEMENT_AUTH_URL', trailingslashit(plugin_dir_url(__FILE__)));
        }
        if (!defined('INTERN_MANAGEMENT_AUTH_PATH')) {
            define('INTERN_MANAGEMENT_AUTH_PATH', trailingslashit(plugin_dir_path(__FILE__)));
        }
    }
    private function load_controllers(): void {
        $path = INTERN_MANAGEMENT_AUTH_PATH . 'app/controllers';
    ...
    private function load_includes(): void{
        $path = INTERN_MANAGEMENT_AUTH_PATH . 'includes';
        if (!is_dir($path)) return;
        foreach (glob($path . '/*.php') as $file) {
            require_once $file;
