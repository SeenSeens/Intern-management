 ...
 */
namespace InternManagement\Modules\Project;
...
final class Project {
    ...
    private function define_constants(): void{
        if(!defined('INTERN_MANAGEMENT_PROJECT_FILE')) {
            define('INTERN_MANAGEMENT_PROJECT_FILE', __FILE__);
        }
        if(!defined('INTERN_MANAGEMENT_PROJECT_URL')) {
            define('INTERN_MANAGEMENT_PROJECT_URL', trailingslashit(plugin_dir_url(__FILE__)));
        }
        if (!defined('INTERN_MANAGEMENT_PROJECT_PATH')) {
            define('INTERN_MANAGEMENT_PROJECT_PATH', trailingslashit(plugin_dir_path(__FILE__)));
        }
    }
    private function load_controllers(): void {
        $path = INTERN_MANAGEMENT_PROJECT_PATH . 'app/controllers';
    ...
    private function load_includes(): void{
        $path = INTERN_MANAGEMENT_PROJECT_PATH . 'includes';
        if (!is_dir($path)) return;
        foreach (glob($path . '/*.php') as $file) {
            require_once $file;
