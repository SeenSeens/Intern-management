 ...
 */
namespace InternManagement\Modules\Report;
...
final class Report {
    ...
    private function define_constants(): void{
        if(!defined('INTERN_MANAGEMENT_REPORT_FILE')) {
            define('INTERN_MANAGEMENT_REPORT_FILE', __FILE__);
        }
        if(!defined('INTERN_MANAGEMENT_REPORT_URL')) {
            define('INTERN_MANAGEMENT_REPORT_URL', trailingslashit(plugin_dir_url(__FILE__)));
        }
        if (!defined('INTERN_MANAGEMENT_REPORT_PATH')) {
            define('INTERN_MANAGEMENT_REPORT_PATH', trailingslashit(plugin_dir_path(__FILE__)));
        }
    }
    private function load_controllers(): void {
        $path = INTERN_MANAGEMENT_REPORT_PATH . 'app/controllers';
    ...
    private function load_includes(): void{
        $path = INTERN_MANAGEMENT_REPORT_PATH . 'includes';
        if (!is_dir($path)) return;
        foreach (glob($path . '/*.php') as $file) {
            require_once $file;
